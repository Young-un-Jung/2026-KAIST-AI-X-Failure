# -*- coding: utf-8 -*-
"""Build the final two-page proposal as an editorial incident dossier.

The HTML is also kept as the design source. Chrome prints two exact A4 sheets;
the same source supports page-by-page screenshot QA through ?page=1 / ?page=2.
"""
...
WORK = ROOT / "proposal-source"
SITE_ASSETS = ROOT / "assets"
...
.sheet {{
  ...
  box-shadow:0 18px 55px rgba(11,20,48,.2);
}}
.sheet::before {{ content:""; position:absolute; left:0; top:0; bottom:0; width:4.5mm; background:var(--ink); }}
...
addEventListener('DOMContentLoaded',()=>{{
  ...
  if(page) document.querySelectorAll('.sheet').forEach((el,i)=>el.style.display=(String(i+1)===page?'block':'none'));
}});
</script>
</head>
<body>
<section class="sheet p1">
  <div class="content">
    <div class="topline mono"><span>FUTURE FAILURE REVIEW · PRE-MORTEM 2036</span><span>TEAM INCOM 45</span></div>
    <div class="eyebrow">2026 KAIST AI × 실패 아이디어 공모전 · 미래에서 온 오답노트</div>
    <h1>그 누구도 위험이라 부르지 않은</h1>
    <p class="subtitle">사람을 위로하던 AI는 왜 한 사람의 고립을 끝내 발견하지 못했는가</p>
    <div class="casebar">
      <div><span class="label">사건 번호</span><span class="value">#2036-0412</span></div>
      <div><span class="label">발생 시점</span><span class="value">2036년 4월</span></div>
      <div><span class="label">조사 상태</span><span class="value status">공식 기록 없음</span></div>
      <div><span class="label">실패 영역</span><span class="value">기술 · 제도 · 관계</span></div>
    </div>
    ...
    <div class="story-grid">
      <div class="story-col">
        <p>AI는 몇 주 전의 고민과 늘 깨어 있던 시간을 기억했고, 지안이 좋아하는 차분한 말투로 대화를 이어갔다. 친구에게 답장하지 않은 지 일주일이 넘었지만 여기서는 핑계를 댈 필요가 없었다. 추궁하지도, 실망하지도 않는 기계 앞에서 지안은 비로소 안도했다.</p>
      ...
      <div class="story-col">
        <p>지안도 한 차례 위기상담 전화를 걸었다. 상담사는 친절했지만 통화는 경찰이나 보호자에게 연락이 갈 수 있다는 고지로 시작됐다. 자신의 밑바닥이 어디까지 기록되고 어떤 파장을 부를지 두려웠던 지안은 끝내 입을 다물고, 아무것도 요구하지 않는 AI에게 돌아갔다.</p>
...
<section class="sheet p2">
  <div class="content">
    ...
    <div class="protocol">
      <article><span class="line-no">LINE 01 · 감지선</span><h3>위험을 다시 정의한다</h3><p>명시적인 위험 문장만 찾는 단기 방어를 넘어, 일상적인 절망과 정서적 의존이 쌓여 가는 만성적 고립을 본다.</p></article>
      <article><span class="line-no">LINE 02 · 전환선</span><h3>상담사에서 연결자로</h3><p>AI는 위로를 대신 끝내거나 핫라인 번호만 띄우지 않는다. 사용자가 현실의 사람에게 닿도록 첫 문장을 건네는 역할로 전환한다.</p></article>
      <article><span class="line-no">LINE 03 · 연결선</span><h3>현실의 느슨한 관계로</h3><p>도서관·청년센터·주민센터의 익명 대화 부스나 가벼운 산책 모임을 자연스럽게 제안한다. AI는 연결자에 머문다.</p></article>
    </div>
...
def run() -> None:
    ...
    if not CHROME.exists():
        raise FileNotFoundError(f"Headless Chrome not found: {CHROME}")
    ...
    if result.returncode != 0 or not ROOT_PDF.exists():
        raise RuntimeError(
            f"PDF build failed ({result.returncode})\n"
